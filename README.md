...
A full-stack tool for engineering teams. Track buying signals across target companies and auto-generate PR digests from your GitHub repos, all in a slick monochrome black UI with floating 3D geometric shapes.
...
- Auto-enrichment: enter a company name or URL and we fetch info automatically
- Nightly scans detect: **Funding**, **Key Hires**, **Layoffs**, **Product Launches**
- Claude AI writes a one-line "why this matters" insight per signal
...
Deploy `frontend/` to Vercel. Set Root Directory to `frontend`. All env vars from `.env.example`.
